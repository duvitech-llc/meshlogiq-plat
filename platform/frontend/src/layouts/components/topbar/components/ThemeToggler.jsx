...
const ThemeToggler = () => {
  ...
  return <div className="topbar-item mx-1">
            <Dropdown>
          ...
          icon: Icon
                            ...
                            <DropdownItem as={'label'} className={clsx('d-flex align-items-center cursor-pointer', {
            active: theme === layoutTheme
          })}>
                                <Icon className="me-2 fs-16" />
                                <span className="flex-grow-1">{toPascalCase(layoutTheme)}</span>
                                <input type="radio" className="form-check-input ms-auto" name="theme" value={layoutTheme} checked={theme === layoutTheme} onChange={() => changeTheme(layoutTheme)} />
                            </DropdownItem>
                        </li>)}
                </DropdownMenu>
            </Dropdown>
